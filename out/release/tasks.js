...
function changelog(branch) {
  ...
  return (0, _streamToPromise2.default)((0, _gulp.src)('./CHANGELOG.md', { buffer: false }).pipe((0, _gulpConventionalChangelog2.default)({
    preset: 'angular' })).pipe((0, _gulp.dest)('./'))).then(() => commitFiles('./CHANGELOG.md', 'Update changelog [ci skip]')).then(() => push(branch));
}
...
function release(options) {
  ...
  return checkStatus().then(() => checkIfTagExists(options)).then(() => getBranch()).then(b => (0, _gulpUtil.log)('On', branch = b, 'branch')).then(() => changelog(branch)).then(() => add(options.addFiles, true)).then(() => checkout('HEAD')).then(() => commitFiles('.', `Version ${options.package.version} for distribution [ci skip]`)).then(() => createNewTag(options.package.version)).then(() => checkout(branch)).then(() => push(branch, true)).then(() => {
    if (branch === 'master') {
